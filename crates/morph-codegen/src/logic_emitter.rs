        ...
        for b in &w.module_bindings {
            ...
            if kind == "import" {
                if let (Some(local), Some(expr)) = (b.get("local"), b.get("expr")) {
                    if !local.is_empty() && !expr.is_empty() {
                        vars.insert(local.clone(), expr.clone());
                    }
                }
                continue;
            }
            ...
            if let (Some(ns), Some(name)) = (b.get("ns"), b.get("name")) {
                if !ns.is_empty() && !name.is_empty() {
                    vars.insert(name.clone(), morph_ir::qualified_binding_ref(ns, name));
                }
            }
        }
...
pub fn generate_state_header(windows: &[IRWindow], premain: &[String]) -> String {
    ...
    // Only legacy flat entries count here: namespaced definitions cannot
    // collide across namespaces, and same-namespace collisions are hard
    // errors at seed time — counting qualified names would only suppress
    // wrappers belonging to other namespaces.
    ...
    // Module functions ARE re-declared here, qualified by namespace: the
    // dev flow never generates `morph_api.h`, and repeated identical
    // declarations are legal C++ where both headers meet.
    let mut func_decls: Vec<(String, String)> = Vec::new();
    let mut seen_decls = HashSet::new();
    for part in premain {
        if let Some((ns, inner)) = split_module_ns(part) {
            if let Some(decl) = extract_function_decl(&inner) {
                if seen_decls.insert((ns.clone(), decl.clone())) {
                    func_decls.push((ns, decl));
                }
            }
            continue;
        }
        if let Some(decl) = extract_function_decl(part) {
            if seen_decls.insert((String::new(), decl.clone())) {
                func_decls.push((String::new(), decl));
            }
        }
    }
    if !func_decls.is_empty() {
        lines.push("// ── JSX functions (defined in the generated TU) ──".to_string());
        emit_shared_entries(&mut lines, func_decls);
        lines.push(String::new());
    }
    lines.join("\n")
}
