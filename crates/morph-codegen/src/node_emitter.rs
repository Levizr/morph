...
const fn js_matching_close(s: &str, open: usize) -> usize {
    let b = s.as_bytes();
...
fn js_object_members(inner: &str, indent: &str) -> String {
    ...
    for p in pairs {
        ...
        let (k, v) = p.find(':').map_or_else(
            || (p.to_string(), "JsUndefined{}".into()),
            |i| (p[..i].trim().to_string(), p[i + 1..].trim().to_string()),
        );
...
fn js_array_elements(inner: &str, indent: &str) -> String {
    ...
    for e in elems {
        ...
        if let Some(arg) = e.strip_prefix("...") {
            ...
            lines.push(format!(
                "{indent}    for (int64_t __i_{idx} = 0; __i_{idx} < (int64_t)__sp_{idx}.length(); ++__i_{idx})"));
            lines.push(format!("{indent}        __a.push(__sp_{idx}[__i_{idx}]);"));
        } else {
...
fn js_is_whole_literal(s: &str, oc: char, cc: char) -> bool {
    if s.is_empty() || !s.starts_with(oc) {
        return false;
    }
...
fn js_transform_leaf(s: &str) -> String {
    ...
    while i < s.len() {
        if s[i..].starts_with(".length") {
            ...
            let _ = write!(out, "(int)({})", js_transform_leaf(&s[left_start..i]));
            out.push_str(".length()");
            i += ".length".len();
            seg = i;
            continue;
...
// at the `.` in `.length`.
const fn js_left_operand_start(text: &str) -> usize {
    let bytes = text.as_bytes();
    let text_len = text.len(); // one past end; we only call with text = text before '.'
                               // text_len points past the object expression; back up one
    ...
    while pos > 0 {
        ...
        if ch == ']' || ch == '}' {
            ...
            while cursor > 0 && depth > 0 {
                cursor -= 1;
                match bytes[cursor] as char {
                    prev if prev == close => depth += 1,
                    prev if prev == open => depth -= 1,
                    _ => {}
                }
            }
            pos = cursor;
...
fn translate_dynamic_expr(expr: &str) -> String {
    let expr = expr.trim();
    if expr.is_empty() {
        return expr.to_string();
    }
    if js_is_whole_literal(expr, '{', '}') || js_is_whole_literal(expr, '[', ']') {
        return translate_js_value(expr);
    }
    ...
    while idx < bytes.len() {
        ...
        if ch == '"' || ch == '\'' {
            idx = js_skip_string(expr, idx);
            continue;
        }
        if ch == '{' || ch == '[' {
            // Is this a literal start (preceded by = : ( , [ { or start)?
            let prev_ok = idx == 0 || {
                let prev_ch = bytes[idx - 1] as char;
                !(prev_ch.is_ascii_alphanumeric()
                    || prev_ch == '_'
                    || prev_ch == ')'
                    || prev_ch == ']'
                    || prev_ch == '}')
            ...
            if prev_ok {
                let close = js_matching_close(expr, idx);
                out.push_str(&js_transform_leaf(&expr[leaf_start..idx]));
                let whole = &expr[idx..=close];
    ...
    out.push_str(&js_transform_leaf(&expr[leaf_start..]));
    out
}
...
pub(crate) fn translate_condition<S: std::hash::BuildHasher>(
    cond: &str,
    state_map: &std::collections::HashMap<String, String, S>,
) -> String {
...
pub fn emit_node_with_state(
    node: &IRNode,
    parent_id: Option<&str>,
    features: &std::collections::HashSet<String>,
    state_map: &std::collections::HashMap<String, String>,
    parent_style: Option<&IRStyle>,
) -> String {
    ...
    match node.node_type.as_str() {
        ...
        "img" => {
            ...
            let alt = node.attrs.get("alt").map_or("", String::as_str);
            lines.push(format!(
    ...
    for ev in &node.events {
        ...
        let rhs = if ev.target.trim_start().starts_with("[&]")
            || ev.target.trim_start().starts_with("[]")
        {
    ...
    for child in &node.children {
        lines.push(emit_node_with_state(
            child,
            Some(&node.node_id),
            features,
            state_map,
...
fn emit_conditional(
    ...
    if !then_code.is_empty() {
        ...
        for l in then_code.lines() {
            lines.push(format!("{bi}        {l}"));
        }
...
fn emit_list(
    ...
    let mut lines = vec![format!(
        "morph::ListContainer* {} = new morph::ListContainer({}, {}, {}, {});",
        node.node_id,
        fmt(node.x),
        fmt(node.y),
        fmt(node.w),
...
fn set_style(
    ...
    let fw = if s.font_weight != "normal" && !s.font_weight.is_empty() {
        ...
        parent
            .map(|p| p.font_weight.clone())
            .filter(|v| v != "normal" && !v.is_empty())
    ...
    let c = if is_text {
        s.color
    } else if s.color != [0.0, 0.0, 0.0, 1.0] {
        s.color
    } else {
        parent
            .map(|p| p.color)
            .filter(|&pc| pc != [0.0, 0.0, 0.0, 1.0])
    ...
    if features.contains("flex") {
        ...
        if s.flex_shrink != 1.0 {
            lines.push(format!("{ind}.flexShrink = {};", fmt(s.flex_shrink)));
        }
        if s.flex_basis != "auto" {
            lines.push(format!("{ind}.flexBasis = \"{}\";", s.flex_basis));
        }
    }
    ...
    // ── OPACITY ──
    if features.contains("opacity") && (s.opacity - 1.0).abs() > f32::EPSILON {
        lines.push(format!("{ind}.opacity = {};", fmt(s.opacity)));
    }
    ...
    if features.contains("scroll") {
        ...
        if s.scrollbar_track_color != [0.85, 0.85, 0.85, 0.4] {
            ...
            lines.push(format!(
                "{ind}.scrollbarTrackColor[1] = {:.4}f;",
                s.scrollbar_track_color[1]
            ));
            lines.push(format!(
                "{ind}.scrollbarTrackColor[2] = {:.4}f;",
                s.scrollbar_track_color[2]
            ));
            lines.push(format!(
                "{ind}.scrollbarTrackColor[3] = {:.4}f;",
                s.scrollbar_track_color[3]
            ));
        }
        if s.scrollbar_thumb_color != [0.5, 0.5, 0.5, 0.6] {
            lines.push(format!(
                "{ind}.scrollbarThumbColor[0] = {:.4}f;",
                s.scrollbar_thumb_color[0]
            ));
            lines.push(format!(
                "{ind}.scrollbarThumbColor[1] = {:.4}f;",
                s.scrollbar_thumb_color[1]
            ));
            lines.push(format!(
                "{ind}.scrollbarThumbColor[2] = {:.4}f;",
                s.scrollbar_thumb_color[2]
            ));
            lines.push(format!(
                "{ind}.scrollbarThumbColor[3] = {:.4}f;",
                s.scrollbar_thumb_color[3]
            ));
        }
...
fn emit_hover_style(
    ...
    let Some(s) = &node.hover_style else {
        return String::new();
    };
    let base = &node.style;
    ...
    {
        o.push(format!("{hv}->opacity = {};", fmt(s.opacity)));
    }
...
fn emit_active_style(
    node: &IRNode,
    indent: &str,
    features: &std::collections::HashSet<String>,
) -> String {
    let Some(s) = &node.active_style else {
        return String::new();
    };
    let base = &node.style;
...
fn emit_transition(node: &IRNode, indent: &str) -> String {
    if node.transition_duration <= 0.0 {
        return String::new();
    }
...
fn emit_animations(
    ...
    if !features.contains("animation") || node.animations.is_empty() {
        return String::new();
    }
    ...
    for a in &node.animations {
        ...
        lines.push(format!(
            "{}{prefix}.push_back(CssAnimation{{\"{}\", {}, {}, {}, {}, {}, {}, {}}});",
            indent,
            a.name,
            fmt(a.duration),
            anim_easing(a),
            fmt(a.delay),
            anim_iterations(a.iterations),
            anim_direction(a),
            anim_fill(a),
            running
        ));
...
fn emit_hover_animations(
    node: &IRNode,
    indent: &str,
    features: &std::collections::HashSet<String>,
) -> String {
    if !features.contains("animation") || node.hover_animations.is_empty() {
        return String::new();
    }
    let mut lines = vec![];
    let hv = format!("{}->hoverStyle", node.node_id);
...
pub(crate) fn css_field_reset(node_var: &str, field_name: &str, indent: &str) -> Vec<String> {
    ...
    let lines: Vec<String> = match field_name {
        "bgColor" => vec!["[0] = 0.0000f;", "[1] = 0.0000f;", "[2] = 0.0000f;", "[3] = 0.0000f;"]
            ...
            .collect(),
        "color" | "borderColor" => {
            vec!["[0] = 0.0000f;", "[1] = 0.0000f;", "[2] = 0.0000f;", "[3] = 1.0000f;"]
                .into_iter()
                .map(|s| format!("{indent}        {prefix}{s}"))
                .collect()
        }
        ...
        "scrollbarThumbColor" => {
            vec!["[0] = 0.5000f;", "[1] = 0.5000f;", "[2] = 0.5000f;", "[3] = 0.6000f;"]
                .into_iter()
                .map(|s| format!("{indent}        {prefix}{s}"))
                .collect()
        }
...
pub(crate) fn css_val_to_cpp(
    ...
    let out: Vec<String> = match val_type {
        ...
        "int" => {
            ...
            let v: f32 =
                if raw.eq_ignore_ascii_case("auto") { 0.0 } else { raw.parse().unwrap_or(0.0) };
            vec![
                format!("{indent}        {prefix} = {};", v as i64),
                format!("{indent}        {node_var}->style.zIndexSet = true;"),
            ]
        }
        "string" => {
...
fn parse_color_val(input: &str) -> Option<(f32, f32, f32, f32)> {
    let lower = input.trim().to_lowercase();
    if let Some(hex) = lower.strip_prefix('#') {
        let (red, green, blue) = match hex.len() {
            3 => {
                let red = u8::from_str_radix(&format!("{}{}", &hex[0..1], &hex[0..1]), 16).ok()?;
                let green =
                    u8::from_str_radix(&format!("{}{}", &hex[1..2], &hex[1..2]), 16).ok()?;
                let blue = u8::from_str_radix(&format!("{}{}", &hex[2..3], &hex[2..3]), 16).ok()?;
                (red, green, blue)
            }
            6 => {
...
fn emit_reactive_effects(
    node: &IRNode,
    indent: &str,
    _features: &std::collections::HashSet<String>,
    state_map: &std::collections::HashMap<String, String>,
) -> String {
    let mut lines = Vec::new();
    let id = &node.node_id;
    ...
    if !node.class_conditional_effects.is_empty() {
        for eff in &node.class_conditional_effects {
            ...
            for (prop, val) in &on_props {
                for a in css_val_to_cpp(id, prop, val, indent) {
                    lines.push(a);
                }
            }
            ...
            if eff.off_styles.is_empty() {
                ...
                for prop in on_props.iter().map(|p| p.0.as_str()) {
                    if let Some((fname, _)) = css_to_style_field(prop) {
                        if !reset_fields.contains(&fname) {
                            reset_fields.push(fname);
                        }
                    }
                }
                reset_fields.sort_unstable();
                for fname in reset_fields {
                    for a in css_field_reset(id, fname, indent) {
                        lines.push(a);
                    }
                }
            } else {
...
pub fn keyframe_registration_code(
    ...
    if !features.contains("animation") || keyframes.is_empty() {
        return String::new();
    }
    let mut lines = vec![
        "// ── @keyframes registry (feature: animation) ──".to_string(),
        "morphClearKeyframes();".to_string(),
    ];
