...
pub(crate) fn print_logo() {
    ...
    println!(
        "  {}  ",
        "Build native OpenGL Applications with TypeScript, TSX, and Morph sources".white().bold()
    );
...
pub(crate) fn log_success(msg: &str) {
...
#[allow(dead_code)]
pub(crate) fn log_header(msg: &str) {
    println!("\n  {}", msg.bold().white());
}

pub(crate) fn log_banner(title: &str) {
    let width = 56;
...
#[allow(dead_code)]
pub(crate) fn log_banner_simple(title: &str) {
    println!();
    println!("  {}", title.bold().white());
    println!();
}

pub(crate) fn log_bullet(msg: &str) {
    println!("      {} {}", "•".cyan().bold(), msg);
}

pub(crate) fn log_muted(msg: &str) {
    println!("      {}", msg.dimmed());
}

pub(crate) fn log_dim(msg: &str) {
    println!("    {} {}", "·".dimmed(), msg.dimmed());
}

pub(crate) fn log_key(label: &str, val: &str) {
    println!("      {}: {}", label.dimmed().bold(), val.bold().white());
}

pub(crate) fn log_section(msg: &str) {
    println!("\n  {}", msg.bold().white());
}

pub(crate) fn log_version(label: &str, current: &str, latest: &str) {
...
pub(crate) fn divider_thick() {
    println!("  {}", "━".repeat(56).dimmed());
}

// ── Indicatif wrappers ──

pub(crate) fn spinner(msg: &str) -> indicatif::ProgressBar {
...
#[allow(dead_code)]
pub(crate) fn progress_bar(len: u64, msg: &str) -> indicatif::ProgressBar {
    let pb = indicatif::ProgressBar::new(len);
    pb.set_style(
        indicatif::ProgressStyle::default_bar()
            .template(
                "    {spinner:.cyan.bold} {msg} [{bar:30.cyan/blue}] {bytes}/{total_bytes} ({eta})",
            )
            .unwrap()
            .progress_chars("█▉▊▋▌▍▎▏ "),
    );
...
pub(crate) fn print_code_frame(file_path: &str, line: usize, col: usize, source_lines: &[String]) {
    let ctx = 2usize;
    let loc = if line > 0 {
        if col > 0 {
    ...
    if source_lines.is_empty() || line == 0 {
        return;
    }
    ...
    for i in start..end {
        ...
        let prefix =
            if is_target { ">".red().bold().to_string() } else { " ".dimmed().to_string() };
...
pub(crate) fn log_lint_errors(
    errors: &[morph_parser::LintError],
    ...
    if n_err > 0 {
        summary += &format!(" {}", format!("{n_err} error(s)").red().bold());
    }
    if n_warn > 0 {
        summary += &format!(" {}", format!("{n_warn} warning(s)").yellow().bold());
    }
    ...
    for e in errors {
        ...
        let label_colored =
            if e.severity == "error" { label.red().bold() } else { label.yellow().bold() };
        println!(
            "\n  {} {} {} {}",
            label_colored,
            ":".dimmed(),
            e.code.bold(),
            format!(": {}", e.message).dimmed()
        );
        ...
        let source_lines = if source_lines.is_empty() {
            std::fs::read_to_string(&e.file_path)
                .ok()
                .map(|c| c.lines().map(ToString::to_string).collect::<Vec<_>>())
                .unwrap_or_default()
        } else {
            source_lines
        };
...
pub(crate) fn suggest_command(input: &str) -> Option<String> {
    let commands = ["new", "install", "update", "dev", "build", "run", "check", "doctor", "cache"];
...
pub(crate) fn did_you_mean(input: &str) {
    if let Some(suggestion) = suggest_command(input) {
        println!("\n    {} Did you mean `{}`?", "?".yellow().bold(), suggestion.cyan().bold());
        println!("    {} Run `morph {} --help` for usage.\n", "→".dimmed(), suggestion);
    } else {
        println!(
