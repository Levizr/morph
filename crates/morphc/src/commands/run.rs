...
pub(crate) fn run(
    ...
    let cfg =
        morph_config::MorphConfig::from_file(&cwd.join("morph.config.json")).unwrap_or_default();
    ...
    if bin_path.exists() {
        ...
        let pb = crate::logger::spinner(&format!(
            "Running {}...",
            bin_path.display().to_string().cyan()
        ));
        ...
        crate::logger::log_warn(&format!(
            "Binary not found at {}. Use `morph run <binary>`.",
            bin_path.display()
        ));
    }
